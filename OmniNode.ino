...
#define defDebugMQTTConnect false                     //
#define defDebugMQTTTransmit false                    //
#define defDebugMQTTReceive false                     //
#define defDebugSensor false                          // Sensor config and reading
#define defDebugTime false                            // time functions
#define defSoftwareVersion "OmniNodeV0_0_7"           // Used in MQTT config report.
...
#define defFuncHMC5883 false                          // defNodeFunction bit 4 (8)
#define defFuncBMP280 false                           // defNodeFunction bit 5 (16)
#define defFuncBMP680 false                           // defNodeFunction bit 6 (32)
#define defFuncMPU6050 true                           // defNodeFunction bit 7 (64)
#define defFuncADXL345 false                          // defNodeFunction bit 8 (128)
#define defFuncAS6500 false                           // defNodeFunction bit 9 (256)
#define defFuncFauxFwd false                          // defNodeFunction bit 10 (512)
// now for brevity, we assemble the flags using bitshifts into a single value
#define defNodeFunction ((defFuncFauxFwd << 9) + (defFuncAS6500 << 8) + (defFuncADXL345 << 7) + (defFuncMPU6050 << 6) + (defFuncBMP680 << 5) + (defFuncBMP280 << 4) + (defFuncHMC5883 << 3) + (defFuncNEO6_7 << 2) + (defFuncINA3221 << 1) + defFuncPacketFwd)
#define defstandardheader(x) \
  (x)["sender"] = strClientID; \
  (x)["wificurr"] = String(WiFi.RSSI()); \
  (x)["time"] = String(CurrentEpochTime()); \
  (x)["msgcount"] = String(intMsgCnt + 1);
...
#endif
#if (defDebugSetup)
#define debugPrSetup(x) Serial.print(x)
#else
#define debugPrSetup(x)
#endif
#if (defDebugFileSys)
...
void setup() {           // initial setup
  ...
  debugPrSetup("Compiled: ");         //
  debugPrSetup(__DATE__);             //
  debugPrSetup(" ");                  //
  debugPrSetup(__TIME__);             //
  ...
  debugPrSetup("Enter compact JSON in Serial Monitor ");                           //
  debugPrSetup("to simulate sending the node a message\n");                        //
  debugPrSetup("EXAMPLE: {\"command\":\"reportinterval\",\"value\":\"3000\"}\n");  //
  setup_wifi();                                                                    //
  instUDP.begin(uintLocalUDPPort);                                                 // start listening in case we need to get MQTT broker by negotiation
